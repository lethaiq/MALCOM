...
class RelGAN_F_TEXTCNN(nn.Module):
    ...
    def forward(self, inp, comments, num_comments, use_dropout=True):
        all_features = []
        content = self.get_content_feature(inp, use_dropout=use_dropout).unsqueeze(1) # batch_size * 1 * feature_dim
        comment = self.encode_comments(comments, use_dropout=use_dropout) # batch_size * (num_comments * seq_len) * feature_dim)

        feature = torch.cat((content, comment), 1).unsqueeze(1) # batch_size * (num_comments * seq_len + 1) * feature_dim
        feature = self.get_feature(feature, use_dropout=use_dropout) # batch_size * feature_dim
    ...
    def encode_comments(self, comments, use_dropout=True):
        ...
        for i in range(comments[0].size()[0]):
            ...
            comment = self.get_comment_feature(comment) # batch_size * max_seq_len * feature_dim
            if use_dropout:
    ...
    def get_comment_feature(self, inp):
