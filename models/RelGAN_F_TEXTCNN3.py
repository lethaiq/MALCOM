...
class RelGAN_F_TEXTCNN3(nn.Module):
    ...
    def encode_comments(self, comments, use_dropout=True):
        ...
        comment = torch.mean(comment.view(len(comment), len(all_comments), cfg.max_seq_len, self.feature_dim), 1) # batch_size * (num_comments * seq_len) * feature_dim)
        return comment
