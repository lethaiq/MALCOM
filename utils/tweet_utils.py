...
urlEnd     = r"(?:\.\.+|[<>]|\s|$)"
url        = regex_or(urlStart1, urlStart2, urlStart3) + urlBody + "(?=(?:"+urlExtraCrapBeforeEnd+")?"+urlEnd+")"
...
numComb  = str(u"[\u0024\u058f\u060b\u09f2\u09f3\u09fb\u0af1\u0bf9\u0e3f\u17db\ua838\ufdfc\ufe69\uff04\uffe0\uffe1\uffe5\uffe6\u00a2-\u00a5\u20a0-\u20b9]?\\d+(?:\\.\\d+)+%?".encode('utf-8'))
...
separators  = "(?:--+|―|—|~|–|=)"
decorations = str(u"(?:[♫♪]+|[★☆]+|[♥❤♡]+|[\u2639-\u263b]+|[\ue001-\uebbb]+)".encode('utf-8'))
...
eeLeft = r"[＼\\ƪԄ\(（<>;ヽ\-=~\*]+"
eeRight= str(u"[\\-=\\);'\u0022<>ʃ）/／ノﾉ丿╯σっµ~\\*]+".encode('utf-8'))
...
emoticon = regex_or(
        ...
        "(?:>|&gt;)?" + str(regex_or(normalEyes, wink) + regex_or(noseArea,"[Oo]") + regex_or(tongue+r"(?=\W|$|RT|rt|Rt)", otherMouths+r"(?=\W|$|RT|rt|Rt)", sadMouths, happyMouths)),

        # reversed version (: D:  use positive lookbehind to remove "(word):"
        # because eyes on the right side is more ambiguous with the standard usage of : ;
        str(regex_or("(?<=(?: ))", "(?<=(?:^))") + regex_or(sadMouths,happyMouths,otherMouths) + noseArea + regex_or(normalEyes, wink) + "(?:<|&lt;)?"),
...
Removed = re.compile(str(regex_or(
                    decorations,
                    # emoticon,
                    Email,
                    Hearts,
...
Emoticon_all = re.compile(emoticon)
...
def remove(phrase):
    phrase = re.sub(Removed, "", phrase)
    phrase = re.sub(Emoticon_all, "", phrase)
    emoji_pattern.sub("", phrase)
    # phrase = re.sub(r"")
    return phrase
...
if __name__ == '__main__':
    text = "I TEST alllll you're we'd do this 'kinds' of 'you're' #hashtags wwwyoutubexiskexkxmw8 and #HASHTAGS, @mentions @trump and :) $3,000 http://t.co/dkfjkdf. w/ <3 :) haha!!!!!"
    # text = "happy birthday, emma stone! raise a glass to her most unfiltered moments"
    # text = "nick viall praises fiancee vanessa grimaldi for raising $30k for special education: 'couldn't be more proud of"
